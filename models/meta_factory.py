...
if __name__ == '__main__':
    ...
    #model = select_model(True,  'MNIST', 'LeNet', 'cpu')
    # model.state_dict = {'flat_w': all values}


    ## Check weights are the same for flattened and unflattened
    # flattened_weights = model.get_param().type(torch.DoubleTensor)
    # flattened_weights_sum = float(torch.sum(flattened_weights))
    # unflattened_weights = model.unflattened_weights()
    # unflattened_weights_sum = float(torch.sum(torch.FloatTensor([torch.sum(v) for k, v in unflattened_weights.items()])))
    # flattened_unflattened_weights = model.flattened_unflattened_weights()
    # flattened_unflattened_weights_sum = float(torch.sum(flattened_unflattened_weights))
    # print(flattened_weights_sum, unflattened_weights_sum, flattened_unflattened_weights_sum)

    ## Check forward with flattened and unflattened weights are the same
    #flattened_weights = model.get_param()
    #unflattened_weights = model.unflattened_weights()

    # for k,v in unflattened_weights.items():
    #     print(k, v.shape)

    x = torch.FloatTensor(256, 1, 28, 28).uniform_(0, 1)
    #x = torch.FloatTensor(256, 3, 32, 32).uniform_(0, 1)

    #logits_flattened = model.forward_with_param(x, flattened_weights)
    #print(float(torch.sum(logits_flattened)))


    #############
    #
    # print(model._weights_numels)
    # print(model._weights_module_names)

    model = select_model(True,  'CIFAR10', 'WRN-16-1', 'cpu')
    ...
    for (name, n) in zip(model._weights_module_names, model._weights_numels):
        #print(name[0], '---', name[1])
        ...
        if isinstance(name[0], torch.nn.Conv2d) and name[1]=='weight':
            layer_name = 'conv{}_W_s{}'.format(layer_count, n)
        elif isinstance(name[0], torch.nn.Conv2d) and name[1]=='bias':
            layer_name = 'conv{}_b_s{}'.format(layer_count, n)
        elif isinstance(name[0], torch.nn.BatchNorm2d) and name[1]=='weight':
            layer_name = 'bn{}_W_s{}'.format(layer_count, n)
        elif isinstance(name[0], torch.nn.BatchNorm2d) and name[1]=='bias':
            layer_name = 'bn{}_b_s{}'.format(layer_count, n)
        elif isinstance(name[0], torch.nn.Linear) and name[1]=='weight':
            layer_name = 'fc{}_W_s{}'.format(layer_count, n)
        elif isinstance(name[0], torch.nn.Linear) and name[1]=='bias':
            layer_name = 'fc{}_b_s{}'.format(layer_count, n)
        else:
            raise ValueError('Unknown layer type {}'.format(name))


        print(layer_name, '---------', 'el{}'.format(el_count))
        el_count+=1
